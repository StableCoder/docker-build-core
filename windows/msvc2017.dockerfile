...
RUN setx /m PATH "%PATH%;C:\Bin" \
    && powershell -command "& { iwr https://aka.ms/vs/15/release/vs_buildtools.exe -OutFile vs_buildtools.exe }" \
    && C:\WorkDir\vs_buildtools.exe --quiet --wait --norestart --nocache --installPath C:\BuildTools \
